...
const bad_words = fs.readFileSync("./filter/regex.txt", 'utf-8');
...
const connect = async (token, retryCount) => {
  try {
    ...
    if (token.length === 0) {
      console.log('Token is empty. Sleep for 5 minutes');
      const access_token = fs.readFileSync(`./tokens/access`, 'utf8');
    ...
    ws.on('message', async (p) => {
      try {
        ...
        if (data.type === 'MESSAGE') {
          ...
          console.log('Received reconnect message from Twitch. Reconnecting to ws...');
          ws.close();
...
const handleMessage = async (data) => {
  ...
  console.log(r.data.redemption.user.display_name);
  console.log("Title: " + title);

  if (title.startsWith(points_config["tts"]["prefix"].length !== 0 ? points_config["tts"]["prefix"] : "TTS")) {
    ...
    const voice = args[1] ? args[1] : voiceList[Math.floor(Math.random() * voiceList.length)];
    const speech = username + " sent a message: " + user_input;
    if (filterMessage(speech)) {
      events.emit('tts', { text: speech, voice });
    }

  } else if (title.startsWith(points_config["sound"]["prefix"].length !== 0 ? points_config["sound"]["prefix"] : "Soundboard: ")) {
    const args = title.split(points_config["sound"]["prefix"].length !== 0 ? points_config["sound"]["prefix"] : "Soundboard: ");
    const soundToPlay = args[1].toLowerCase();
    const sounds = await getSoundEffects();
    const soundEffect = sounds.find(
      n => (n.name === soundToPlay ||
        n.aliases.includes(soundToPlay)) &&
        (!n.events || n.events.includes('sfx'))
    );
    if (soundEffect) {
      events.emit('sfx', { soundToPlay, soundEffect, volume: soundEffect.volume });
    }
  } else if (points_config["redemptions"]["redeemables"].findIndex(item => title.toLowerCase() === item.toLowerCase()) !== -1) {
    const username = r.data.redemption.user.display_name;
...
const t = fs.readFileSync(`./tokens/access`, 'utf8');
connect(t, 3);
