...
const connect = async (token, retryCount) => {
  try {
    const channel_id = fs.readFileSync(`./tokens/id`, 'utf8');
    ...
    if (token.length === 0 || channel_id.length === 0) {
      console.log('Token or channel_id is empty. Please verify. Sleep for 5 minutes');
...
const handleMessage = async (data) => {
  ...
  const tts_prefix = points_config["tts"]["prefix"].length !== 0 ? points_config["tts"]["prefix"] : "TTS";
  const sound_prefix = points_config["sound"]["prefix"].length !== 0 ? points_config["sound"]["prefix"] : "Soundboard: ";
  ...
  if (title.toLowerCase().startsWith(tts_prefix.toLowerCase())) {
    ...
    const soundEffect = sounds.find(
      n => (n.name.toLowerCase() === soundToPlay ||
